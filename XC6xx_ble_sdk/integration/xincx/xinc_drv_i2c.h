...
#ifndef XINC_DRV_I2C_H__
#define XINC_DRV_I2C_H__
...
#include <xincx.h>
    ...
    #define XINC_I2C_ERROR_ADDRESS_NACK  XINC_I2CM_ERROR_ADDRESS_NACK
    #define XINC_I2C_ERROR_DATA_NACK     XINC_I2CM_ERROR_DATA_NACK
    #define XINC_I2C_FREQ_100K           XINC_I2CM_FREQ_100K
    #define XINC_I2C_FREQ_250K           XINC_I2CM_FREQ_250K
    #define XINC_I2C_FREQ_400K           XINC_I2CM_FREQ_400K
#endif
...
typedef enum
{
    XINC_DRV_I2C_FREQ_100K = XINC_I2C_FREQ_100K , ///< 100 kbps.
    XINC_DRV_I2C_FREQ_400K = XINC_I2C_FREQ_400K   ///< 400 kbps.
 ...
 */
#define XINC_DRV_I2C_DEFAULT_CONFIG                                               \
{                                                                                \
...
#define XINC_DRV_I2C_FLAG_TX_POSTINC          (1UL << 0) /**< TX buffer address incremented after transfer. */
#define XINC_DRV_I2C_FLAG_RX_POSTINC          (1UL << 1) /**< RX buffer address incremented after transfer. */
#define XINC_DRV_I2C_FLAG_NO_XFER_EVT_HANDLER (1UL << 2) /**< Interrupt after each transfer is suppressed, and the event handler is not called. */
#define XINC_DRV_I2C_FLAG_HOLD_XFER           (1UL << 3) /**< Set up the transfer but do not start it. */
#define XINC_DRV_I2C_FLAG_REPEATED_XFER       (1UL << 4) /**< Flag indicating that the transfer will be executed multiple times. */
#define XINC_DRV_I2C_FLAG_TX_NO_STOP          (1UL << 5) /**< Flag indicating that the TX transfer will not end with a stop condition. */
...
typedef enum
{
    XINC_DRV_I2C_EVT_DONE,         ///< Transfer completed event.
    XINC_DRV_I2C_EVT_ADDRESS_NACK, ///< Error event: NACK received after sending the address.
    XINC_DRV_I2C_EVT_DATA_NACK     ///< Error event: NACK received after sending a data byte.
...
typedef enum
{
    XINC_DRV_I2C_XFER_TX,          ///< TX transfer.
    XINC_DRV_I2C_XFER_RX,          ///< RX transfer.
    XINC_DRV_I2C_XFER_TXRX,        ///< TX transfer followed by RX transfer with repeated start.
    XINC_DRV_I2C_XFER_TXTX         ///< TX transfer followed by TX transfer with repeated start.
...
#define XINC_DRV_I2C_XFER_DESC_TX(addr, p_data, length)                 \
    {                                                                  \
...
#define XINC_DRV_I2C_XFER_DESC_RX(addr, p_data, length)                 \
    {                                                                  \
...
#define XINC_DRV_I2C_XFER_DESC_TXRX(addr, p_tx, tx_len, p_rx, rx_len)   \
    {                                                                  \
...
#define XINC_DRV_I2C_XFER_DESC_TXTX(addr, p_tx, tx_len, p_tx2, tx_len2) \
    {                                                                  \
 ...
 * - @ref XINC_DRV_I2C_XFER_TXRX<span></span>: Write operation followed by a read operation (without STOP condition in between).
 * - @ref XINC_DRV_I2C_XFER_TXTX<span></span>: Write operation followed by a write operation (without STOP condition in between).
 * - @ref XINC_DRV_I2C_XFER_TX<span></span>:   Write operation (with or without STOP condition).
 * - @ref XINC_DRV_I2C_XFER_RX<span></span>:   Read operation  (with STOP condition).
 *
...
#define XINC_DRV_I2C_WITH_I2C
#elif !defined(I2C_PRESENT) && defined(I2CM_PRESENT)
#define XINC_DRV_I2C_WITH_I2CM
#else
...
#if defined(XINC_DRV_I2C_WITH_I2CM) && defined(XINC_DRV_I2C_WITH_I2C)
    #define XINC_DRV_I2C_USE_I2CM  (p_instance->use_easy_dma)
#elif defined(XINC_DRV_I2C_WITH_I2CM)
    #define XINC_DRV_I2C_USE_I2CM  true
#else
    #define XINC_DRV_I2C_USE_I2CM  false
#endif
#define XINC_DRV_I2C_USE_I2C  (!XINC_DRV_I2C_USE_I2CM)
