...
void bsp_evt_handler(bsp_event_t evt)
{
    ...
    switch (evt)
    {
        ...
        {
            //点亮 LED 指示灯 D1
            bsp_board_led_on(bsp_board_pin_to_led_idx(LED_1));      printf("%s,BSP_EVENT_LED1_ON=%d\n",__func__,BSP_EVENT_LED1_ON);   
        } break;
        // 按键S1 注册的释放事件的回调
        case BSP_EVENT_LED1_OFF:
        {
            //熄灭 LED 指示灯 D1
            bsp_board_led_off(bsp_board_pin_to_led_idx(LED_1));printf("%s,BSP_EVENT_LED1_OFF=%d\n",__func__,BSP_EVENT_LED1_OFF);   
        } break;
        // 按键S2 注册的长按事件的回调
        case BSP_EVENT_LED2_ON:
        {
            //点亮 LED 指示灯 D2
            bsp_board_led_on(bsp_board_pin_to_led_idx(LED_2));printf("%s,BSP_EVENT_LED2_ON=%d\n",__func__,BSP_EVENT_LED2_ON);   
        } break;
        // 按键S2 注册的释放事件的回调
        case BSP_EVENT_LED2_OFF:
        {
            //熄灭 LED 指示灯 D2
            bsp_board_led_off(bsp_board_pin_to_led_idx(LED_2));printf("%s,BSP_EVENT_LED2_ON=%d\n",__func__,BSP_EVENT_LED2_ON);   
        } break;
