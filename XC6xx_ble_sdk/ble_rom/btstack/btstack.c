...
extern btstack_packet_handler_t  att_handler ;
...
{
	if(PATCH_FUN[BLE_STACK_INIT_ID]){
         ((void    (*)(void *init))PATCH_FUN[BLE_STACK_INIT_ID])(init);
         return ;
    }
    ...
    hci_event_callback_registration2.callback = btstack_init->packet_handler;
    hci_add_event_handler(&hci_event_callback_registration2);
