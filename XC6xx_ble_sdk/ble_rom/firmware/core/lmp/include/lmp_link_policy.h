...
t_devicelink* LMpol_Get_Next_Master_Tx_Activity(void);
...
#if (PRH_BS_CFG_SYS_LMP_SNIFF_SUPPORTED==1)     //- LC add (#if #endif) for turn off SNIFF.
void LMpol_Adjust_Wakeup_Slot_Time(t_lmp_link *p_link, u_int32 instant);
#else
#define LMpol_Adjust_Wakeup_Slot_Time(p_link, instant)
#endif
...
#if (PRH_BS_CFG_SYS_LMP_SNIFF_SUPPORTED==1)     //- LC add (#if #else #endif) in order to disable SNIFF.
boolean LMpol_Sniff_Activation_On_Next_Frame(void);
#else
#define LMpol_Sniff_Activation_On_Next_Frame(void)  0
#endif
