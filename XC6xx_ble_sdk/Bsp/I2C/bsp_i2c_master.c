...
extern	void	i2c_master_init(uint8_t	i2cAddR, uint32_t speed)
{
    ...
    if(speed == I2C_FAST_MODE) {
        ...
        __write_hw_reg32(I2C_FS_SCL_HCNT, 0x0c);
        __write_hw_reg32(I2C_FS_SCL_LCNT, 0x13); 
    }
    else if(speed == I2C_STANDARD_MODE) {

        __write_hw_reg32(I2C_CON, 0x63); 
        __write_hw_reg32(I2C_SS_SCL_HCNT, 0x48);
        __write_hw_reg32(I2C_SS_SCL_LCNT, 0x4f); 
    }
...
{ 
    ...
    gpio_mux_ctl(2,0);gpio_fun_inter(2,0);gpio_fun_sel(2,5); //scl  5:I2C_SCL
    gpio_mux_ctl(3,0);gpio_fun_inter(3,0);gpio_fun_sel(3,6); //sda  6:I2C_SDA
    i2c_master_init(0xa0 >>1, I2C_STANDARD_MODE);//0x50=0xA0>>1
    while(at24cxx_check());//24c08
    at24cxx_write(001,(uint8_t*)TEXT_Buf,SIZE);
	at24cxx_read(001,datatemp,SIZE);
	printf("%s",datatemp);
    while(1);
