...
XINC_SECTION_DEF(cli_command, xinc_cli_cmd_entry_t);
#define CLI_DATA_SECTION_ITEM_GET(i) XINC_SECTION_ITEM_GET(cli_command, xinc_cli_cmd_entry_t, (i))
#define CLI_DATA_SECTION_ITEM_COUNT  XINC_SECTION_ITEM_COUNT(cli_command, xinc_cli_cmd_entry_t)
...
#define XINC_CLI_VT100_CMD(_p_cli_, _cmd_)   {       \
    ...
    xinc_fprintf(_p_cli_->p_fprintf_ctx, "%s", cmd); \
}
...
static bool cli_log_entry_process(xinc_cli_t const * p_cli, bool skip);
static void cli_execute(xinc_cli_t const * p_cli);


static inline void transport_buffer_flush(xinc_cli_t const * p_cli)
{
    xinc_fprintf_buffer_flush(p_cli->p_fprintf_ctx);
}

static inline void cli_flag_help_set(xinc_cli_t const * p_cli)
{
    p_cli->p_ctx->internal.flag.show_help = 1;
}
static inline void cli_flag_help_clear(xinc_cli_t const * p_cli)
{
    p_cli->p_ctx->internal.flag.show_help = 0;
}

#if XINC_MODULE_ENABLED(XINC_CLI_BUILD_IN_CMDS)
static inline void cli_flag_echo_set(xinc_cli_t const * p_cli)
{
    p_cli->p_ctx->internal.flag.echo = 1;
}

static inline void cli_flag_echo_clear(xinc_cli_t const * p_cli)
{
    p_cli->p_ctx->internal.flag.echo = 0;
}
#endif /* XINC_MODULE_ENABLED(XINC_CLI_BUILD_IN_CMDS) */

static inline bool cli_flag_echo_is_set(xinc_cli_t const * p_cli)
{
    return p_cli->p_ctx->internal.flag.echo == 1 ? true : false;
}

static inline bool cli_flag_processing_is_set(xinc_cli_t const * p_cli)
{
    return p_cli->p_ctx->internal.flag.processing == 1 ? true : false;
}

static inline uint8_t cli_flag_last_nl_get(xinc_cli_t const * p_cli)
{
    return p_cli->p_ctx->internal.flag.last_nl;
}

static inline void cli_flag_last_nl_set(xinc_cli_t const * p_cli, uint8_t val)
{
    p_cli->p_ctx->internal.flag.last_nl = val;
}

static inline void receive_state_change(xinc_cli_t const * p_cli, xinc_cli_receive_t state)
{
...
static void cli_cmd_buffer_clear(xinc_cli_t const * p_cli)
{
...
static inline bool cursor_in_empty_line(xinc_cli_t const * p_cli)
{
...
static inline bool full_line_cmd(xinc_cli_t const * p_cli)
{
...
static void cli_write(xinc_cli_t const * p_cli,
                      void const *      p_data,
                      size_t            length,
                      size_t *          p_cnt)
{
...
static inline void cli_putc(xinc_cli_t const * p_cli, char ch)
{
    xinc_fprintf(p_cli->p_fprintf_ctx, "%c", ch);
}

/* Function reads data from the CLI instance. */
static void cli_read(xinc_cli_t const * p_cli,
                     void *            p_data,
                     size_t            length,
                     size_t *          p_cnt)
{
 ...
 */
static void cmd_get(xinc_cli_cmd_entry_t const *     p_command,
                    size_t                          lvl,
                    size_t                          idx,
                    xinc_cli_static_entry_t const ** pp_entry,
                    xinc_cli_static_entry_t *        p_st_entry)
{
    ...
    if (lvl == XINC_CLI_CMD_ROOT_LVL)
    {
        ...
        {
            xinc_cli_cmd_entry_t const * p_cmd = NULL;
 ...
 * =>                   cur_y = 2, cur_y_end = 3
 */
static xinc_cli_multiline_cons_t const * multiline_console_data_check(xinc_cli_t const * p_cli)
{
    xinc_cli_ctx_t * p_ctx = p_cli->p_ctx;
    xinc_cli_multiline_cons_t * p_cons = &p_cli->p_ctx->vt100_ctx.cons;
...
static uint32_t cli_line_num_with_buffer_offset_get(xinc_cli_t const * p_cli,
                                                    xinc_cli_cmd_len_t buffer_pos)
{
    uint32_t name_len;
    xinc_cli_multiline_cons_t *p_cons = &p_cli->p_ctx->vt100_ctx.cons;
...
static uint32_t cli_col_num_with_buffer_offset_get(xinc_cli_t const * p_cli,
                                                   xinc_cli_cmd_len_t buffer_pos)
{
    uint32_t name_len;
    xinc_cli_multiline_cons_t *p_cons = &p_cli->p_ctx->vt100_ctx.cons;
...
static int32_t cli_row_span_with_buffer_offsets_get(xinc_cli_t const * p_cli,
                                                    xinc_cli_cmd_len_t offset1,
                                                    xinc_cli_cmd_len_t offset2)
{
...
static int32_t cli_column_span_with_buffer_offsets_get(xinc_cli_t const * p_cli,
                                                       xinc_cli_cmd_len_t offset1,
                                                       xinc_cli_cmd_len_t offset2)
{
...
static inline void cursor_next_line_move(xinc_cli_t const * p_cli)
{
    xinc_fprintf(p_cli->p_fprintf_ctx, "\n");
}

/* Function moves cursor left by n positions. */
static inline void cursor_left_move(xinc_cli_t const * p_cli, xinc_cli_cmd_len_t n)
{
    if (n > 0)
    {
        xinc_fprintf(p_cli->p_fprintf_ctx, "\033[%dD", n);
    }
}

/* Function moves cursor right by n positions. */
static inline void cursor_right_move(xinc_cli_t const * p_cli, xinc_cli_cmd_len_t n)
{
    if (n > 0)
    {
        xinc_fprintf(p_cli->p_fprintf_ctx, "\033[%dC", n);
    }
}

#if XINC_MODULE_ENABLED(XINC_CLI_METAKEYS)
/* Moves cursor horizontally by a number. Positive is right */
static void cursor_horiz_move(xinc_cli_t const * p_cli, int32_t delta)
{
    if (delta > 0)
    {
...
static inline void cursor_up_move(xinc_cli_t const * p_cli, xinc_cli_cmd_len_t n)
{
    if (n > 0)
    {
        xinc_fprintf(p_cli->p_fprintf_ctx, "\033[%dA", n);
    }
}
...
static inline void cursor_down_move(xinc_cli_t const * p_cli, xinc_cli_cmd_len_t n)
{
    if (n > 0)
    {
         xinc_fprintf(p_cli->p_fprintf_ctx, "\033[%dB", n);
    }
}

#if XINC_MODULE_ENABLED(XINC_CLI_METAKEYS)
/* Moves cursor vertically by a number. Positive is down */
static void cursor_vert_move(xinc_cli_t const * p_cli, int32_t delta)
{
    if (delta > 0)
    {
...
static void cursor_position_increment(xinc_cli_t const * p_cli)
{
    ...
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
    xinc_cli_cmd_len_t cur_y = p_cons->cur_y;
...
static void cursor_position_synchronize(xinc_cli_t const * p_cli)
{
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
 ...
 */
static void cli_cmd_word_remove(xinc_cli_t const * p_cli)
{
    xinc_cli_cmd_len_t new_pos;
    xinc_cli_cmd_len_t chars_to_delete;
    int32_t row_span;
    int32_t col_span;
...
static void cursor_home_position_move(xinc_cli_t const * p_cli)
{
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
...
static void cursor_end_position_move(xinc_cli_t const * p_cli)
{
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
...
static ret_code_t cursor_position_get(xinc_cli_t const * p_cli)
{
    size_t   cnt;
    ...
    xinc_fprintf(p_cli->p_fprintf_ctx, cmd_get_terminal_size);
...
//                xinc_delay_us(999);
            ...
            if (c == 'R') /* end of response from the terminal */
            {
                ...
                {
                    p_cli->p_ctx->vt100_ctx.cons.cur_x = (xinc_cli_cmd_len_t)x;
                }
                ...
                {
                    p_cli->p_ctx->vt100_ctx.cons.cur_y = (xinc_cli_cmd_len_t)y;
                }
                p_cli->p_ctx->temp_buff[0] = 0;
                return XINC_SUCCESS;
            }
...
static ret_code_t terminal_size_get(xinc_cli_t const *   p_cli,
                                    xinc_cli_cmd_len_t * p_length,
                                    xinc_cli_cmd_len_t * p_height)
{
    ASSERT(p_length);
    ASSERT(p_height);
...
static void vt100_color_set(xinc_cli_t const * p_cli, xinc_cli_vt100_color_t color)
{
    if (color != XINC_CLI_DEFAULT)
    {
        ...
        xinc_fprintf(p_cli->p_fprintf_ctx, "%s", cmd);
    }
    else
    {
...
static void vt100_bgcolor_set(xinc_cli_t const * p_cli, xinc_cli_vt100_color_t bgcolor)
{
...
static inline void vt100_colors_store(xinc_cli_t const *        p_cli,
                                      xinc_cli_vt100_colors_t * p_color)
{
    memcpy(p_color, &p_cli->p_ctx->vt100_ctx.col, sizeof(xinc_cli_vt100_colors_t));
}

static void vt100_colors_restore(xinc_cli_t const *              p_cli,
                                 xinc_cli_vt100_colors_t const * p_color)
{
...
static void left_arrow_handle(xinc_cli_t const * p_cli)
{
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
...
static void right_arrow_handle(xinc_cli_t const * p_cli)
{
    xinc_cli_multiline_cons_t const * p_cons = multiline_console_data_check(p_cli);
...
static inline void  char_insert_echo_off(xinc_cli_t const * p_cli, char data)
{
...
static void char_insert(xinc_cli_t const * p_cli, char data)
{
    xinc_cli_cmd_len_t diff;
...
static void char_backspace(xinc_cli_t const * p_cli)
{
    xinc_cli_cmd_len_t diff;
    ...
    else
    {
        ...
        xinc_fprintf(p_cli->p_fprintf_ctx, "%s", cmd_bspace);
    }
}

static void char_delete(xinc_cli_t const * p_cli)
{
    xinc_cli_cmd_len_t diff;
...
static void cli_state_set(xinc_cli_t const * p_cli, xinc_cli_state_t state)
{
    ...
    if (state == XINC_CLI_STATE_ACTIVE)
    {
            ...
            xinc_cli_fprintf(p_cli, XINC_CLI_INFO, "%s", p_cli->p_name);
    }
}

#if XINC_MODULE_ENABLED(XINC_CLI_HISTORY)
static inline void history_mode_exit(xinc_cli_t const * p_cli)
{
    p_cli->p_ctx->p_cmd_list_element = NULL;
}

static void history_handle(xinc_cli_t const * p_cli, bool up)
{
    xinc_cli_memobj_header_t header = {
        .p_prev = NULL,
        .p_next = NULL,
        .cmd_len = 0
    };
    xinc_cli_cmd_len_t current_cmd_len;
    bool skip = false;
    ...
    if (!up) /* button down */
    {
        ...
        xinc_memobj_read(p_cli->p_ctx->p_cmd_list_element,
                        &header,
    ...
    xinc_cli_fprintf(p_cli, XINC_CLI_NORMAL, "%s", p_cli->p_ctx->cmd_buff);
    if (cursor_in_empty_line(p_cli) || full_line_cmd(p_cli))
    {
        cursor_next_line_move(p_cli);
    }
...
static void history_list_element_add(xinc_cli_t const * p_cli, xinc_memobj_t * p_memobj)
{
    ...
    {
        xinc_memobj_read(p_cli->p_ctx->p_cmd_list_head,
                        &header,
                        XINC_CLI_HISTORY_HEADER_SIZE,
                        0);
        ...
        xinc_memobj_write(p_cli->p_ctx->p_cmd_list_head,
                         &header,
                         XINC_CLI_HISTORY_HEADER_SIZE,
                         0);
...
static void history_list_element_oldest_remove(xinc_cli_t const * p_cli)
{
    ...
    xinc_cli_memobj_header_t header;
    xinc_memobj_t * p_memobj = p_cli->p_ctx->p_cmd_list_tail;

    xinc_memobj_read(p_memobj,
                    &header,
                    XINC_CLI_HISTORY_HEADER_SIZE,
                    0);

    p_cli->p_ctx->p_cmd_list_tail = header.p_next;
    memset(&header, 0, sizeof(xinc_cli_memobj_header_t));
    xinc_memobj_write(p_memobj, &header, XINC_CLI_HISTORY_HEADER_SIZE, 0);
    xinc_memobj_free(p_memobj);
    ...
    xinc_memobj_read(p_cli->p_ctx->p_cmd_list_tail,
                    &header,
                    XINC_CLI_HISTORY_HEADER_SIZE,
                    0);

    header.p_prev = NULL;
    xinc_memobj_write(p_cli->p_ctx->p_cmd_list_tail, &header, XINC_CLI_HISTORY_HEADER_SIZE, 0);
}

static void history_list_free_memory(xinc_cli_t const * p_cli)
{
...
static void history_save(xinc_cli_t const * p_cli)
{
    xinc_cli_cmd_len_t cmd_new_len = cli_strlen(p_cli->p_ctx->cmd_buff);
    ...
    {
        xinc_memobj_t * p_memobj;

        p_memobj = xinc_memobj_alloc(p_cli->p_cmd_hist_mempool,
...
static xinc_cli_cmd_len_t str_similarity_check(char const * str_a, char const * str_b)
{
    xinc_cli_cmd_len_t cnt = 0;
...
static void completion_insert(xinc_cli_t const * p_cli,
                              char const *      p_compl,
                              xinc_cli_cmd_len_t compl_len)
{
    ASSERT (p_compl);

    xinc_cli_cmd_len_t diff = p_cli->p_ctx->cmd_buff_len - p_cli->p_ctx->cmd_buff_pos;
...
static void option_print(xinc_cli_t const * p_cli,
                         char const *      p_option,
                         xinc_cli_cmd_len_t longest_option)
{
    ...
    xinc_cli_cmd_len_t columns =
        (p_cli->p_ctx->vt100_ctx.cons.terminal_wid - cli_strlen(tab)) / longest_option;
    xinc_cli_cmd_len_t diff = longest_option - cli_strlen(p_option);

    if (p_cli->p_ctx->vt100_ctx.printed_cmd++ % columns == 0)
    {
        xinc_cli_fprintf(p_cli, XINC_CLI_OPTION, "\n%s%s", tab, p_option);
    }
    else
    {
        xinc_cli_fprintf(p_cli, XINC_CLI_OPTION, "%s", p_option);
    }
...
static void cli_tab_handle(xinc_cli_t const * p_cli)
{
    size_t cmd_idx;
    size_t cmd_last = 0;
    size_t cmd_first = 0;
    ...
    xinc_cli_cmd_len_t cmd_lvl = XINC_CLI_CMD_ROOT_LVL;
    xinc_cli_cmd_len_t cmd_longest = 0; /* longest matching command */

    /* Calculating the longest possible completion length. -1 for '\0'. */
    xinc_cli_cmd_len_t compl_len = (XINC_CLI_CMD_BUFF_SIZE - 1) - p_cli->p_ctx->cmd_buff_len;
    ...
    xinc_cli_cmd_len_t arg_len = cli_strlen(argv[cmd_lvl]);

    /* Variable 'static_entry' is needed to handle dynamic commands. */
    xinc_cli_static_entry_t static_entry;

    xinc_cli_cmd_entry_t const * p_cmd = NULL;
    xinc_cli_static_entry_t const * p_st_cmd = NULL;
    xinc_cli_static_entry_t const * p_st_cmd_last = NULL;
    ...
    do
    {
            ...
            (cmd_lvl >= argc - 1 + space))
        {
            ...
            while (1)
            {
                ...
                {
                    xinc_cli_cmd_len_t len = cli_strlen(p_st_cmd->p_syntax);
                    if (len > cmd_longest)
                    {
                        cmd_longest = len;
                    }
                    ...
                    {
                        xinc_cli_static_entry_t last_entry;
                ...
                {
                    xinc_cli_warn(p_cli, XINC_CLI_MSG_TAB_OVERFLOWED);
                    break;
                }
            }
        }
    ...
    xinc_cli_fprintf(p_cli, XINC_CLI_INFO, "\n%s", p_cli->p_name);
    xinc_cli_fprintf(p_cli, XINC_CLI_NORMAL, "%s", p_cli->p_ctx->cmd_buff);
...
static bool process_nl(xinc_cli_t const * p_cli, uint8_t data)
{
    if ((data != '\r') && (data != '\n'))
    {
...
static void cli_state_collect(xinc_cli_t const * p_cli)
{
    size_t count = 0;
    char data;
...
static void cmd_trim(xinc_cli_t const * p_cli)
{
    xinc_cli_cmd_len_t i = 0;
...
{
    xinc_cli_cmd_len_t shift = 0;
    xinc_cli_cmd_len_t len = cli_strlen(p_char);
    ...
    for (xinc_cli_cmd_len_t i = 0; i < len - 1; i++)
    {
        if (isspace((int)p_char[i]))
        {
            for (xinc_cli_cmd_len_t j = i + 1; j < len; j++)
            {
...
static bool command_to_tmp_buffer_add(xinc_cli_t const * p_cli,
                                      char const *      p_new_cmd,
                                      char const *      p_pattern)
{
    xinc_cli_cmd_len_t cmd_len = cli_strlen(p_new_cmd);
    xinc_cli_cmd_len_t shift;
...
static void pattern_from_tmp_buffer_remove(xinc_cli_t const * p_cli,
                                           char const *      p_pattern)
{
    size_t shift;
    char * p_pattern_addr = strstr(p_cli->p_ctx->temp_buff, p_pattern);

    xinc_cli_cmd_len_t pattern_len = cli_strlen(p_pattern);
 ...
 *
 * This function is internal to xinc_cli module and shall be not called directly.
 *
 ...
 */
static wildcard_cmd_status_t commands_expand(xinc_cli_t const *           p_cli,
                                             xinc_cli_cmd_entry_t const * p_cmd,
                                             size_t                      cmd_lvl,
                                             char *                      p_pattern,
                                             size_t *                    p_counter)
{
    ...
    xinc_cli_static_entry_t         static_entry;
    xinc_cli_static_entry_t const * p_static_entry = NULL;
...
*/
    ...
    if (quote != 0)
    {
        xinc_cli_error(p_cli, "not terminated: %c", quote);
        return;
    }
    ...
    for (cmd_idx = 0; cmd_idx <= CLI_DATA_SECTION_ITEM_COUNT; ++cmd_idx)
    {
        ...
        {
            xinc_cli_error(p_cli, "%s%s", argv[0], XINC_CLI_MSG_COMMAND_NOT_FOUND);
            return;
        }
            ...
            if (p_static_entry->handler != NULL)
            {
                ...
                if (commands_expanded > 0)
                {
                    ...
                    * with a handler to avoid multiple function calls. */
                    xinc_cli_error(p_cli, "Error: requested multiple function executions");
    ...
    {
        xinc_cli_error(p_cli, XINC_CLI_MSG_SPECIFY_SUBCOMMAND);
    }
    cli_flag_help_clear(p_cli);
}
...
static void cli_transport_evt_handler(xinc_cli_transport_evt_t evt_type, void * p_context)
{
    xinc_cli_t * p_cli = (xinc_cli_t *)p_context;
    ASSERT(p_cli);
#if XINC_MODULE_ENABLED(XINC_CLI_USES_TASK_MANAGER)
...
static ret_code_t xinc_cli_instance_init(xinc_cli_t const * p_cli,
                                        void const *      p_config,
                                        bool              use_colors)
{
    ASSERT(p_cli);
    ...
    ret = xinc_memobj_pool_init(p_cli->p_cmd_hist_mempool);
    if (ret != XINC_SUCCESS)
    {
        return ret;
    }
    ...
    memset(p_cli->p_ctx, 0, sizeof(xinc_cli_ctx_t));
    p_cli->p_ctx->internal.flag.tx_rdy = 1;
    ...
    for (size_t i = 0; i < CLI_DATA_SECTION_ITEM_COUNT; i++)
    {
        const xinc_cli_cmd_entry_t * cmd;
...
#if XINC_MODULE_ENABLED(XINC_CLI_USES_TASK_MANAGER)
static ret_code_t xinc_cli_instance_uninit(xinc_cli_t const * p_cli);
void console_task(void * p_context)
{
    xinc_cli_t * p_cli = (xinc_cli_t *)p_context;

    ret_code_t ret = xinc_cli_start(p_cli);
    APP_ERROR_CHECK(ret);
    ...
    while (1)
    {
        ...
        {
            (void)xinc_cli_instance_uninit(p_cli);
            task_exit();
        }
        else
        {
            xinc_cli_process(p_cli);
        }
    }
}
#endif
                        ...
                        xinc_log_severity_t init_lvl)
{
    ASSERT(p_cli);
		printf("%s\r\n",__func__);
    ret_code_t err_code = xinc_cli_instance_init(p_cli, p_config, use_colors);

#if XINC_CLI_LOG_BACKEND && XINC_MODULE_ENABLED(XINC_LOG)
    if ((err_code == XINC_SUCCESS) && log_backend && XINC_CLI_LOG_BACKEND)
    {
        int32_t id = xinc_log_backend_add(p_cli->p_log_backend, init_lvl);
        if (id < 0)
        {
            return XINC_ERROR_NO_MEM;
        }

        xinc_log_backend_enable(p_cli->p_log_backend);
    }
#endif
    return err_code;
}
...
static ret_code_t xinc_cli_instance_uninit(xinc_cli_t const * p_cli)
{
    ...
    {
        xinc_log_backend_disable(p_cli->p_log_backend);
        xinc_log_backend_remove(p_cli->p_log_backend);
    }
#endif
...
ret_code_t xinc_cli_uninit(xinc_cli_t const * p_cli)
{
    ...
    return XINC_SUCCESS;
#else
    return xinc_cli_instance_uninit(p_cli);
#endif
}
...
ret_code_t xinc_cli_start(xinc_cli_t const * p_cli)
{
    ...
    ((xinc_cli_log_backend_t *)p_cli->p_log_backend->p_ctx)->p_context = p_context;
#endif
...
void xinc_cli_process(xinc_cli_t const * p_cli)
{
    ...
    xinc_cli_internal_t internal;
    internal.value = 0;
    internal.flag.processing = 1;
    (void)xinc_atomic_u32_or((xinc_atomic_u32_t *)&p_cli->p_ctx->internal.value,
                            internal.value);
		static  uint32_t state = 0xff;
		if(state != p_cli->p_ctx->state)
		{
			printf("cli_process state:%d\r\n",p_cli->p_ctx->state);
		}
    ...
    (void)xinc_atomic_u32_and((xinc_atomic_u32_t *)&p_cli->p_ctx->internal.value,
                             internal.value);
}

/* Function shall be only used by the xinc_fprintf module. */
void xinc_cli_print_stream(void const * p_user_ctx, char const * p_data, size_t data_len)
{
    cli_write((xinc_cli_t const *)p_user_ctx,
              p_data,
              data_len,
              NULL);
}
                                            ...
                                            ...)
{
        ...
        (color != p_cli->p_ctx->vt100_ctx.col.col))
    {
        ...
        xinc_fprintf_fmt(p_cli->p_fprintf_ctx, p_fmt, &args);
 ...
 */
static void format_offset_string_print(xinc_cli_t const * p_cli,
                                       char const *      p_str,
                                       ...
                                       bool              offset_first_line)
{
    ...
    while (1)
    {
        ...
        if (length <= p_cli->p_ctx->vt100_ctx.cons.terminal_wid - terminal_offset)
        {
            ...
            xinc_fprintf(p_cli->p_fprintf_ctx, p_str + offset);
            break;
        }
        else
        {
                        ...
                        size_t                          opt_len)
{
    ...
    xinc_cli_static_entry_t static_entry;
    xinc_cli_cmd_entry_t const * p_cmd = p_cli->p_ctx->active_cmd.p_subcmd;
    xinc_cli_static_entry_t const * p_st_cmd = NULL;
    ...
    xinc_cli_print(p_cli, "Subcommands:");
    ...
    while (1)
    {
        ...
        field_width = longest_string + tab_len;
        xinc_cli_fprintf(p_cli, XINC_CLI_NORMAL,"  %-*s:", field_width, p_st_cmd->p_syntax);
...
static bool cli_log_entry_process(xinc_cli_t const * p_cli, bool skip)
{
    xinc_log_entry_t entry;

#if XINC_MODULE_ENABLED(XINC_CLI_STATISTICS)
    bool print_msg = false;
#endif

    if (xinc_queue_pop(((xinc_cli_log_backend_t *)p_cli->p_log_backend->p_ctx)->p_queue, &entry) !=
                                                                                      XINC_SUCCESS)
    {
        return false;
    }
    ...
    if (print_msg)
    {
        ...
        xinc_cli_error(p_cli, "Lost logs - increase log backend queue size.");
    ...
    do
    {
        ...
        xinc_memobj_read(entry, &header, HEADER_SIZE*sizeof(uint32_t), memobj_offset);
        ...
        if (header.base.generic.type == HEADER_TYPE_STD)
        {
            ...
            xinc_memobj_read(entry, args, nargs*sizeof(uint32_t), memobj_offset);
        ...
        else if (header.base.generic.type == HEADER_TYPE_HEXDUMP)
        {
            ...
            do
            {
                ...
                xinc_memobj_read(entry, data_buf, chunk_len, memobj_offset);
                memobj_offset += chunk_len;
                data_len      -= chunk_len;
                xinc_log_hexdump_entry_process(data_buf, chunk_len, &params, p_cli->p_fprintf_ctx);
            } while (data_len > 0);
        }

        xinc_memobj_put(entry);
    } while (xinc_queue_pop(((xinc_cli_log_backend_t *)p_cli->p_log_backend->p_ctx)->p_queue, &entry)
                                                                                    == XINC_SUCCESS);
    return true;
}

static void xinc_log_backend_cli_put(xinc_log_backend_t const * p_backend, xinc_log_entry_t * p_msg)
{
    xinc_cli_log_backend_t * p_backend_cli = (xinc_cli_log_backend_t *)p_backend->p_ctx;
    xinc_cli_t const * p_cli = p_backend_cli->p_cli;
    ...
    if (p_cli->p_ctx->state != XINC_CLI_STATE_PANIC_MODE_INACTIVE)
    {
        ...
        ret_code_t err_code = xinc_queue_push(p_backend_cli->p_queue, &p_msg);
        while (err_code != XINC_SUCCESS)
        {
            (void)cli_log_entry_process(p_cli, panic_mode ? false : true);

            err_code = xinc_queue_push(p_backend_cli->p_queue, &p_msg);
        }
...
static void xinc_log_backend_cli_flush(xinc_log_backend_t const * p_backend)
{
    xinc_cli_log_backend_t * p_backend_cli = (xinc_cli_log_backend_t *)p_backend->p_ctx;
    xinc_cli_t const *       p_cli = p_backend_cli->p_cli;
...
static void xinc_log_backend_cli_panic_set(xinc_log_backend_t const * p_backend)
{
    xinc_cli_log_backend_t * p_backend_cli = (xinc_cli_log_backend_t *)p_backend->p_ctx;
    xinc_cli_t const * p_cli = p_backend_cli->p_cli;
...
const xinc_log_backend_api_t xinc_log_backend_cli_api = {
        .put       = xinc_log_backend_cli_put,
        .flush     = xinc_log_backend_cli_flush,
        .panic_set = xinc_log_backend_cli_panic_set,
};
#else
static bool cli_log_entry_process(xinc_cli_t const * p_cli, bool skip)
{
...
static bool xinc_cli_build_in_cmd_common_executed(xinc_cli_t const *               p_cli,
                                                 bool                            arg_cnt_nok,
                                                 xinc_cli_getopt_option_t const * p_opt,
                                                 size_t                          opt_len)
{
    ...
    {
        xinc_cli_help_print(p_cli, p_opt, opt_len);
        return true;
    }

    if (arg_cnt_nok)
    {
         xinc_cli_error(p_cli, "%s: wrong parameter count", p_cli->p_ctx->active_cmd.p_syntax);
         return true;
    }
...
static void xinc_cli_cmd_clear(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ASSERT(p_cli);
    ASSERT(p_cli->p_ctx && p_cli->p_iface && p_cli->p_name);
    UNUSED_PARAMETER(argv);

    if ((argc == 2) && (xinc_cli_help_requested(p_cli)))
    {
...
static void xinc_cli_cmd_cli(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ASSERT(p_cli);
    ASSERT(p_cli->p_ctx && p_cli->p_iface && p_cli->p_name);
    UNUSED_PARAMETER(argv);
...
#if XINC_MODULE_ENABLED(XINC_CLI_VT100_COLORS)
static void xinc_cli_cmd_colors_off(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
    }
...
static void xinc_cli_cmd_colors_on(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
...
static void xinc_cli_cmd_colors(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ASSERT(p_cli);
    ASSERT(p_cli->p_ctx && p_cli->p_iface && p_cli->p_name);
    ...
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 2), NULL, 0))
    {
        return;
    }

    xinc_cli_error(p_cli, "%s:%s%s", argv[0], XINC_CLI_MSG_UNKNOWN_PARAMETER, argv[1]);
}
#endif // XINC_MODULE_ENABLED(XINC_CLI_VT100_COLORS)

static void xinc_cli_cmd_echo(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc > 2), NULL, 0))
    {
        return;
    ...
    xinc_cli_print(p_cli, "Echo status: %s", cli_flag_echo_is_set(p_cli) ? "on" : "off");
}

static void xinc_cli_cmd_echo_off(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
...
static void xinc_cli_cmd_echo_on(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
...
#if XINC_MODULE_ENABLED(XINC_CLI_HISTORY)
static void xinc_cli_cmd_history(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ASSERT(p_cli);
    ASSERT(p_cli->p_ctx && p_cli->p_iface && p_cli->p_name);
    ...
    size_t i = 0;
    xinc_memobj_t const * p_cmd_list = p_cli->p_ctx->p_cmd_list_tail;
    xinc_cli_memobj_header_t header;
    ...
    while (1)
    {
        ...
        xinc_memobj_read((xinc_memobj_t * )p_cmd_list,
                        &header,
                        XINC_CLI_HISTORY_HEADER_SIZE,
                        0);
        xinc_memobj_read((xinc_memobj_t * )p_cmd_list,
        ...
        xinc_cli_print(p_cli, "[%3d] %s", i++, p_cli->p_ctx->temp_buff);
    }
    p_cli->p_ctx->temp_buff[0] = '\0';
}
#endif // XINC_MODULE_ENABLED(XINC_CLI_HISTORY)
...
#if XINC_MODULE_ENABLED(XINC_CLI_STATISTICS)
void xinc_cli_cmd_cli_stats(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (argc == 1)
    {
        xinc_cli_help_print(p_cli, NULL, 0);
    ...
    UNUSED_RETURN_VALUE(xinc_cli_build_in_cmd_common_executed(p_cli, (argc > 2), NULL, 0));
}

void xinc_cli_cmd_cli_stats_show(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
    ...
    xinc_queue_t const  * p_queue = ((xinc_cli_log_backend_t *)p_cli->p_log_backend->p_ctx)->p_queue;
    uint8_t max_util = xinc_queue_max_utilization_get(p_queue);
    uint8_t utilization = (uint8_t)(max_util * 100ul / p_queue->size);

    xinc_cli_print(p_cli,
...
void xinc_cli_cmd_cli_stats_reset(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
    ...
    xinc_queue_max_utilization_reset(
                               ((xinc_cli_log_backend_t *)p_cli->p_log_backend->p_ctx)->p_queue);
}
#endif // XINC_MODULE_ENABLED(XINC_CLI_STATISTICS)

static void xinc_cli_cmd_resize_default(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    if (xinc_cli_build_in_cmd_common_executed(p_cli, (argc != 1), NULL, 0))
    {
        return;
...
static void xinc_cli_cmd_resize(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ASSERT(p_cli);
    ASSERT(p_cli->p_ctx && p_cli->p_iface && p_cli->p_name);
    ...
    if (argc == 1)
    {
                              ...
                              &p_cli->p_ctx->vt100_ctx.cons.terminal_hei) != XINC_SUCCESS)
        {
            p_cli->p_ctx->vt100_ctx.cons.terminal_wid = XINC_CLI_DEFAULT_TERMINAL_WIDTH;
            p_cli->p_ctx->vt100_ctx.cons.terminal_hei = XINC_CLI_DEFAULT_TERMINAL_HEIGHT;
            xinc_cli_warn(p_cli, "No response from the terminal, assumed 80x24 screen size");
        }
        return;
    }
...
{
    XINC_CLI_CMD(off,    NULL,   XINC_CLI_HELP_ECHO_OFF,          xinc_cli_cmd_echo_off),
    XINC_CLI_CMD(on,     NULL,   XINC_CLI_HELP_ECHO_ON,           xinc_cli_cmd_echo_on),
    XINC_CLI_SUBCMD_SET_END
};
...
{
    XINC_CLI_CMD(reset,  NULL,   XINC_CLI_HELP_STATISTICS_RESET,  xinc_cli_cmd_cli_stats_reset),
    XINC_CLI_CMD(show,   NULL,   XINC_CLI_HELP_STATISTICS_SHOW,   xinc_cli_cmd_cli_stats_show),
    XINC_CLI_SUBCMD_SET_END
};
#endif // XINC_MODULE_ENABLED(XINC_CLI_STATISTICS)
...
{
    XINC_CLI_CMD(default, NULL, XINC_CLI_HELP_RESIZE_DEFAULT, xinc_cli_cmd_resize_default),
    XINC_CLI_SUBCMD_SET_END
};

XINC_CLI_CMD_REGISTER(clear,     NULL,           XINC_CLI_HELP_CLEAR,     xinc_cli_cmd_clear);
XINC_CLI_CMD_REGISTER(cli,       &m_sub_cli,     XINC_CLI_HELP_CLI,       xinc_cli_cmd_cli);
#if XINC_MODULE_ENABLED(XINC_CLI_HISTORY)
XINC_CLI_CMD_REGISTER(history,   NULL,           XINC_CLI_HELP_HISTORY,   xinc_cli_cmd_history);
#endif
XINC_CLI_CMD_REGISTER(resize,    &m_sub_resize,  XINC_CLI_HELP_RESIZE,    xinc_cli_cmd_resize);
