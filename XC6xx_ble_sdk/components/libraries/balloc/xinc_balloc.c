...
XINC_SECTION_DEF(xinc_balloc, xinc_balloc_t);

#if XINC_BALLOC_CLI_CMDS && XINC_CLI_ENABLED
#include "xinc_cli.h"

static void xinc_balloc_status(xinc_cli_t const * p_cli, size_t argc, char **argv)
{
    ...
    uint32_t num_of_instances = XINC_SECTION_ITEM_COUNT(xinc_balloc, xinc_balloc_t);
    uint32_t i;

    for (i = 0; i < num_of_instances; i++)
    {
        const xinc_balloc_t * p_instance = XINC_SECTION_ITEM_GET(xinc_balloc, xinc_balloc_t, i);
        ...
        uint32_t max_util   = xinc_balloc_max_utilization_get(p_instance);
        uint32_t util       = xinc_balloc_utilization_get(p_instance);
...
XINC_CLI_CREATE_STATIC_SUBCMD_SET(xinc_balloc_commands)
{
     XINC_CLI_CMD(status, NULL, "Print status of balloc instances.", xinc_balloc_status),
     XINC_CLI_SUBCMD_SET_END
};

XINC_CLI_CMD_REGISTER(balloc, &xinc_balloc_commands, "Commands for BALLOC management", xinc_balloc_status);
 ...
 */
__STATIC_INLINE void * xinc_balloc_block_unwrap(xinc_balloc_t const * p_pool, void * p_head)
{
 ...
 */
__STATIC_INLINE void * xinc_balloc_element_wrap(xinc_balloc_t const * p_pool, void * p_element)
{
    ASSERT((p_pool    != NULL) && ((p_pool->block_size % sizeof(uint32_t))    == 0));
 ...
 * @return      Pointer to the beginning of the block.
 */
static void * xinc_balloc_idx2block(xinc_balloc_t const * p_pool, uint8_t idx)
{
    ASSERT(p_pool != NULL);
 ...
 */
static uint8_t xinc_balloc_block2idx(xinc_balloc_t const * p_pool, void const * p_block)
{
    ASSERT(p_pool != NULL);
...
void * xinc_balloc_alloc(xinc_balloc_t const * p_pool)
{
    ...
    {
        // Allocate block.
        p_block = xinc_balloc_idx2block(p_pool, *--(p_pool->p_cb->p_stack_pointer));
    ...
    if (p_block != NULL)
    {
        p_block = xinc_balloc_block_unwrap(p_pool, p_block);
    }
#endif
...
void xinc_balloc_free(xinc_balloc_t const * p_pool, void * p_element)
{
...
#if XINC_BALLOC_CONFIG_DEBUG_ENABLED
    void * p_block = xinc_balloc_element_wrap(p_pool, p_element);
    ...
    if (XINC_BALLOC_DEBUG_DOUBLE_FREE_CHECK_GET(p_pool->debug_flags))
    {
        ...
        {
            if (xinc_balloc_idx2block(p_pool, *p_idx) == p_block)
            {
    ...
    // Free the element.
    *(p_pool->p_cb->p_stack_pointer)++ = xinc_balloc_block2idx(p_pool, p_block);
