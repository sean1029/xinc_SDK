...
}xinc_strerror_desc_t;

/**
 * @brief Unknown error code.
 *
 * The constant string used by @ref xinc_strerror_get when the error description was not found.
 */
 ...
 */
static xinc_strerror_desc_t const xinc_strerror_array[] =
{
...
char const * xinc_strerror_get(ret_code_t code)
{
    char const * p_ret = xinc_strerror_find(code);
    return (p_ret == NULL) ? m_unknown_str : p_ret;
}

char const * xinc_strerror_find(ret_code_t code)
{
    xinc_strerror_desc_t const * p_start;
    xinc_strerror_desc_t const * p_end;
    p_start = xinc_strerror_array;
    p_end   = xinc_strerror_array + ARRAY_SIZE(xinc_strerror_array);

    while (p_start < p_end)
    {
        xinc_strerror_desc_t const * p_mid = p_start + ((p_end - p_start) / 2);
