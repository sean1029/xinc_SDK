...
/**
 * @defgroup xinc_section_iter Section variables iterator
 * @ingroup app_common
 * @{
 */
...
{
#if defined(__GNUC__)
    xinc_section_t           section;    //!< Description of the set of sections.
                                        /**<
                                         ...
                                         */
#else
    xinc_section_t const   * p_first;    //!< Pointer to the first section in the set.
    xinc_section_t const   * p_last;     //!< Pointer to the last section in the set.
#endif
...
typedef struct
{
    xinc_section_set_t const * p_set;        //!< Pointer to the appropriate section set.
#if !defined(__GNUC__)
    xinc_section_t const     * p_section;    //!< Pointer to the selected section.
                                            /**<
                                             * In case of GCC all sections in the set are sorted and
...
#define XINC_SECTION_SET_DEF(_name, _type, _count)                                                   \
    ...
    static xinc_section_set_t const _name =                                                          \
    {                                                                                               \
        .section =                                                                                  \
        {                                                                                           \
...
static xinc_section_t const CONCAT_2(_name, _array)[] =                                              \
{                                                                                                   \
 ...
 */
static inline void * xinc_section_iter_get(xinc_section_iter_t const * p_iter)
{
    ASSERT(p_iter);
