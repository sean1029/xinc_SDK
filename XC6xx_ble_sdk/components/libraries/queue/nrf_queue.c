...
XINC_SECTION_DEF(nrf_queue, nrf_queue_t);
...
static void nrf_queue_status(nrf_cli_t const * p_cli, size_t argc, char **argv)
{
    ...
    uint32_t num_of_instances = XINC_SECTION_ITEM_COUNT(nrf_queue, nrf_queue_t);
    uint32_t i;

    for (i = 0; i < num_of_instances; i++)
    {
        const nrf_queue_t * p_instance = XINC_SECTION_ITEM_GET(nrf_queue, nrf_queue_t, i);
...
XINC_CLI_CREATE_STATIC_SUBCMD_SET(nrf_queue_commands)
{
     XINC_CLI_CMD(status, NULL, "Print status of queue instances.", nrf_queue_status),
     XINC_CLI_SUBCMD_SET_END
};

XINC_CLI_CMD_REGISTER(queue, &nrf_queue_commands, "Commands for BALLOC management", nrf_queue_status);
