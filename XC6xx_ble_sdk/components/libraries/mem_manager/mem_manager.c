...
#if XINC_MODULE_ENABLED(MEM_MANAGER)
#include <stdio.h>
#include "mem_manager.h"
#include "xinc_assert.h"

#define XINC_LOG_MODULE_NAME mem_mngr

#if MEM_MANAGER_CONFIG_LOG_ENABLED
#define XINC_LOG_LEVEL       MEM_MANAGER_CONFIG_LOG_LEVEL
#define XINC_LOG_INFO_COLOR  MEM_MANAGER_CONFIG_INFO_COLOR
#define XINC_LOG_DEBUG_COLOR MEM_MANAGER_CONFIG_DEBUG_COLOR
 ...
 *
 * @retval (XINC_ERROR_NULL | XINC_ERROR_MEMORY_MANAGER_ERR_BASE) when @ref PARAM is NULL.
 */
#define NULL_PARAM_CHECK(PARAM)                            \
    if ((PARAM) == NULL)                                   \
    {                                                      \
        return (XINC_ERROR_NULL | XINC_ERROR_MEMORY_MANAGER_ERR_BASE); \
    }
 ...
 *        Returning with an appropriate error code on failure.
 *
 * @retval (XINC_ERROR_INVALID_STATE | XINC_ERROR_MEMORY_MANAGER_ERR_BASE) module is uninitialized.
 */
#define VERIFY_MODULE_INITIALIZED()                                     \
    ...
    {                                                                   \
        ...
        {                                                               \
            return (XINC_ERROR_INVALID_STATE | XINC_ERROR_MEMORY_MANAGER_ERR_BASE); \
        }                                                               \
    } while (0)
 ...
 *
 * @retval (XINC_ERROR_INVALID_PARAM | XINC_ERROR_MEMORY_MANAGER_ERR_BASE) if requested size is greater
...
#define VERIFY_REQUESTED_SIZE(SIZE)                                     \
    ...
    {                                                                   \
        ...
        {                                                               \
            return (XINC_ERROR_INVALID_PARAM | XINC_ERROR_MEMORY_MANAGER_ERR_BASE); \
        }                                                               \
    } while (0)
...
static const uint32_t m_min_size_default[BLOCK_CAT_COUNT] =
{
    MEMORY_MANAGER_XXSMALL_BLOCK_SIZE,
    MEMORY_MANAGER_XSMALL_BLOCK_SIZE,
    MEMORY_MANAGER_SMALL_BLOCK_SIZE,
...
static void block_allocate(uint32_t block_index)
{
    ...
    {
        XINC_LOG_INFO("%d: %d -> %d", block_cat, m_peak_count[block_cat], m_cur_count[block_cat]);
...
{
    XINC_LOG_DEBUG(">> %s.", (uint32_t)__func__);
    ...
    XINC_MEM_MANAGER_DIAGNOSE_RESET;

#if (MEM_MANAGER_DISABLE_API_PARAM_CHECK == 0)
    m_module_initialized = true;
#endif // MEM_MANAGER_DISABLE_API_PARAM_CHECK

    XINC_MEM_MANAGER_DIAGNOSE;
...
uint32_t xinc_mem_reserve(uint8_t ** pp_buffer, uint32_t * p_size)
{
    ...
    XINC_LOG_DEBUG(">> %s, size 0x%04lX.", (uint32_t)__func__, requested_size);
    ...
    uint32_t       err_code     = (XINC_ERROR_NO_MEM | XINC_ERROR_MEMORY_MANAGER_ERR_BASE);

    XINC_LOG_DEBUG("Start index for the pool = 0x%08lX, total block count 0x%08X",
           block_index,
           TOTAL_BLOCK_COUNT);
    ...
    for (; block_index < TOTAL_BLOCK_COUNT; block_index++)
    {
        ...
        {
            XINC_LOG_DEBUG("Reserving block 0x%08lX", block_index);
    ...
    {
        XINC_LOG_ERROR("Memory reservation failed: err_code %d, memory %p, size %d!",
                err_code,
                (uint32_t)(*pp_buffer),
                (*p_size));
    ...
    XINC_LOG_DEBUG("<< %s %p, result 0x%08lX.", (uint32_t)__func__,
...
void * xinc_malloc(uint32_t size)
{
    ...
    uint32_t retval = xinc_mem_reserve(&buffer, &allocated_size);

    if (retval != XINC_SUCCESS)
    {
        XINC_LOG_ERROR("Failed to malloc side: %d", size);
        printf("Failed to malloc side: %d\r\n", size);
        buffer = NULL;
    }
...
void * xinc_calloc(uint32_t count, uint32_t size)
{
    uint8_t * buffer = NULL;
    uint32_t allocated_size = (size * count);

    XINC_LOG_DEBUG("[%s]: Requested size %d, count %d", (uint32_t)__func__, allocated_size, count);

    uint32_t retval = xinc_mem_reserve(&buffer, &allocated_size);
    if (retval == XINC_SUCCESS)
    {
        XINC_LOG_DEBUG("[%s]: buffer %p, total size %d", (uint32_t)__func__, (uint32_t)buffer, allocated_size);
        memset(buffer,0, allocated_size);
    }
    else
    {
        XINC_LOG_ERROR("[%s]: Failed to allocate memory %d", (uint32_t)__func__, allocated_size);
        buffer = NULL;
    }
...
void xinc_free(void * p_mem)
{
    VERIFY_MODULE_INITIALIZED_VOID();
    NULL_PARAM_CHECK_VOID(p_mem);

    XINC_LOG_DEBUG(">> %s %p.", (uint32_t)__func__, (uint32_t)p_mem);
    ...
    for (index = 0; index < TOTAL_BLOCK_COUNT; index++)
    {
        if (&m_memory[memory_index] == p_mem)
        {
            ...
            XINC_LOG_DEBUG("<< Freeing block %d.", index);
            block_init(index);
            break;
        }
...
void * xinc_realloc(void * p_mem, uint32_t size)
{
    return p_mem;
}
...
void xinc_mem_diagnose(void)
{
    ...
    XINC_LOG_INFO("");
    XINC_LOG_INFO("+------------+------------+------------+------------+------------+------------+");
    XINC_LOG_INFO("| Block      | Size       | Total      | In Use     | Min Alloc  | Max Alloc  |");
    XINC_LOG_INFO("+------------+------------+------------+------------+------------+------------+");

    printf("\r\n");
    printf("+------------+------------+------------+------------+------------+------------+------------+\r\n");
    printf("| Block      | Size       | Total cnt  | In Use     | Min Alloc  | Max Alloc  |\r\n");
    printf("+------------+------------+------------+------------+------------+------------+------------+\r\n");
...
void xinc_mem_diagnose_reset(void)
{
    ...
    memset(&m_peak_count, 0, sizeof(m_peak_count));
    memset(&m_cur_count, 0, sizeof(m_cur_count));
}

#endif // MEM_MANAGER_ENABLE_DIAGNOSTICS
/** @} */
#endif //XINC_MODULE_ENABLED(MEM_MANAGER)
